...
def main():
    ...
    import fnmatch
    import os
    import tamtam.tamtam as tamtam
...
- a filepath or URL""")
    ...
    parser.add_option("--job-id", dest = "job_id", default = '', help = "Set jobId for messages. app-ver.si.on is a good one")
    parser.add_option("--tamtam-spamlevel", dest = "tamtam_spamlevel", type="int", default = 2, help = "Set jobId for messages. app-ver.si.on is a good one")
    parser.add_option("--tamtam-token", dest = "tamtam_token", default = None, help = "Set tamtam token.")
    parser.add_option("-a", "--alarm-level", dest = "alarm_level", type="int", default = 0, help = "Just to print an alarm level logline")
    parser.add_option("-m", "--mask", dest = "fmask", default = '*.*', help = "File mask for directory searches.")
    ...
    API_KEY = "79fa0d5673781814d5d1032afdcab66d10868f482b3cf204cffd0c4e39fe6530"

    api_key = options.api_key or API_KEY

    ALARM_LEVEL = options.alarm_level
    ...
    for argument in arguments:
        ...
        # patch windows UNC path. It may lead to a bugs with screening symbols
        if argument.find('\\') == 0:
        ...
        if os.path.isdir(argument):
            print("[INFO] scanning %s path..." % argument)
            for root, dirnames, filenames in os.walk(argument):       
                for filename in fnmatch.filter(filenames, options.fmask):
                    resources.append(os.path.join(root, filename))
        else:
            print("[INFO] scanning %s file..." % argument)
    ...
    def analyze(resource):
        try:
            if action.lower() == "scan":
                report = v.scan(resource, reanalyze = False)
            ...
            print "VirusTotal returned a non correct response. It may be because the script does too many requests at the minute. See the parameter -l", e
    ...
    while not q.empty():
        ...
        if report.positives > ALARM_LEVEL:
            print "[ALARM] Too much positives to release this binary"
            if options.tamtam_spamlevel > 0:
                post_msg = "[ALARM] %s : %d/%d positives matched in %s\n%s\n@@all" %\
                        (options.job_id, report.positives, report.total, resource, report.permalink)           
        elif report.positives > 0:
            print "[WARN] some positives found"
            if options.tamtam_spamlevel > 1:
                post_msg = "[WARN] %s : %d/%d positives matched in %s\n%s\n" %\
                        (options.job_id, report.positives, report.total, resource, report.permalink)
        else:
            print "[CLEAN]"
            if options.tamtam_spamlevel > 2:
                post_msg = "[CLEAN] %s : %d/%d positives matched in %s\n%s\n" %\
                        (options.job_id, report.positives, report.total, resource, report.permalink)

        if options.tamtam_token is not None and post_msg is not None:
            try:
                tamtam.send_message(tamtam.do_wrap_links(post_msg),
                                   options.tamtam_token, 'virustotal scan results')
